...
feature_mat_files <- list.files(in_pre, pattern = paste0(in_sample,".+sparse_matrix.tsv.gz"), full.names = TRUE)
feature_mat_names <- sub(paste0(in_sample,"_(.+)_sparse_matrix.tsv.gz"),"\\1", basename(feature_mat_files))
...
#### Check window sizes
```{r}
window_mat_files <- list.files(in_pre, pattern = paste0(in_sample, ".+window.+counts.tsv.gz"), full.names = TRUE)
...
```{r}
frag_widths_file <- list.files(in_pre, pattern = paste0(in_sample, ".+fragment_widths.tsv.gz"), full.names = TRUE)
