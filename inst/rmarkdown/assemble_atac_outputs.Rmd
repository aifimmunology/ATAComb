...
if(is.null(params$in_post)) {
  in_post <- system.file("testdata/preprocessed", package = "ATAComb")
...
stm(paste0("IN  preprocessed dir          : ", in_post))
...
```{r Print Arguments}
print(c(
  paste0("IN  preprocessed dir          : ", in_post),
...
if(!dir.exists(in_post)) {
  stm(paste0("ERROR: Cannot find IN preprocessed dir:", in_post))
  stop()
}
...
```{r}
frag_widths_file <- list.files(in_post, pattern = paste0(in_sample, ".+fragment_widths.tsv.gz"), full.names = TRUE)
