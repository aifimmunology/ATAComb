...
if(grepl("per_barcode_metrics.csv", in_pbm)) {
  ...
  if(! "cell_uuid" %in% names(tenx_meta)) {
    stop("arc qc requires addition of a cell_uuid. See 00_arc_uuid.sh in tenx-atacseq-pipeline.")
  }
  
  tenx_meta <- tenx_meta[, list(barcode, atac_raw_reads, atac_dup_reads, atac_mitochondrial_reads, atac_fragments, cell_uuid)]
  names(tenx_meta) <- c("original_barcodes","n_fragments","n_duplicate","n_mito","n_unique","cell_uuid")
}
...
names(meta)[names(meta) == "cell_uuid"] <- "barcodes"

meta$atac_cell_uuid <- ids::uuid(nrow(meta),
                                 drop_hyphens = TRUE, 
                                 use_time = TRUE)
meta$atac_cell_name <- ids::adjective_animal(nrow(meta),
                                             n_adjectives = 2,
                                             max_len = 10)
```
...
filtered_meta <- filtered_meta[match(filtered_archr_barcodes, filtered_meta$original_barcodes),]
