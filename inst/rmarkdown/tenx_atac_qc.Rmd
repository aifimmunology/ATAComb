...
if(!dir.exists(in_tenx)) {
  ...
  arc_sin <- file.path(in_tenx, "arc_atac_singlecell.csv")
  atac_sin <- file.path(in_tenx, "singlecell.csv")
  bc_sin <- file.path(in_tenx, "barcode_counts.tsv")
  
  if(file.exists(arc_sin)) {
    stm(paste("Found Arc singlecell.csv file:", arc_sin))
    in_sin <- arc_sin
  ...
  arc_sum <- file.path(in_tenx, "arc_atac_summary.csv")
  atac_sum <- file.path(in_tenx, "summary.csv")
  
  if(file.exists(arc_sum)) {
    stm(paste("Found Arc summary.csv file:", arc_sum))
    in_sum <- arc_sum
  } else if(file.exists(atac_sum)) {
    stm(paste("Found ATAC summary.csv file:", atac_sum))
    in_sum <- atac_sum
  } else {
    in_sum <- NULL
  ...
  arc_frag <- file.path(in_tenx, "atac_fragments.tsv.gz")
  atac_frag <- file.path(in_tenx, "fragments.tsv.gz")
  
  if(file.exists(arc_frag)) {
    stm(paste("Found Arc fragments.tsv.gz file:", arc_frag))
    in_frag <- arc_frag
  } else if(file.exists(atac_frag)) {
    stm(paste("Found ATAC fragments.tsv.gz file:", atac_frag))
    in_frag <- atac_frag
  } else {
...
feature_mat_names <- sub("_sparse_matrix.tsv.gz","",feature_mat_files)
feature_mat_names <- sub(".+_", "", feature_mat_names)
...
window_mat_names <- sub("_counts.tsv.gz","",window_mat_files)
window_mat_names <- sub(".+_", "", window_mat_names)
...
# cellranger-atac uses "None" in cell_ids
# cellranger-arc metadata that's been pre-processed will already have UUIds in this column.
# barcode_counts.tsv-based datasets will have "None" filled into the barcodes column.
if(tenx_meta$barcodes[1] == "None") {
    tenx_meta$barcodes <- ids::uuid(nrow(tenx_meta),
                                    drop_hyphens = TRUE, 
                                    use_time = TRUE)
}
...
archr_meta <- archr_meta %>%
  dplyr::mutate(singlet = DoubletEnrichment >= 1)

n_singlets <- sum(meta$singlet)

stm(paste0("ArchR: Singlet barcodes = ", n_singlets))

n_doublets <-  sum(!meta$singlet)
...
filtered_meta$Singlet <- arch_meta[["singlet"]]
