...
filtered_archr_names <- getCellNames(proj)
filtered_archr_barcodes <- sub(".+#","",archr_names)
...
proj <- subsetArchRProject(proj,
                           cells = archr_names,
                           outputDirectory = paste0(temp_archr_dir,"subset"))
...
h5_MetaNames <- h5_MetaNames[h5_MetaNames$dim != length(archr_names),]

h5_CellNames <- h5read(out_arrow, "/Metadata/CellNames")
h5_keepLogic <- h5_CellNames %in% archr_barcodes
...
Replace original cell names with barcodes and PBMC ID
```{r}
stm(paste0("Updating barcodes in ", out_arrow))
h5_CellNames <- h5_ls[h5_ls$name == "CellNames",]
...
h5delete(out_arrow, "/Metadata/Sample")
h5write(filtered_meta$pbmc_sample_id[1], out_arrow, "/Metadata/Sample")
```
...
system(paste("rm -r", paste0(temp_archr_dir,"subset")))
if(out_dir != "./") {
