...
addArchRThreads(parallel::detectCores(), force = TRUE)
...
if(!dir.exists(in_tenx)) {
  ...
  in_sin <- file.path(in_tenx, "singlecell.csv")
  if(!file.exists(in_sin)) {
    stm(paste0("ERROR: Cannot find 10x summary.csv:", in_sin))
    stop()
  }
  ...
  if(!file.exists(in_frag)) {
    stm(paste0("ERROR: Cannot find 10x fragments.tsv.gz:", in_tenx))
    stop()
  }
}
...
stm(paste(c("Matching reference matrices:", matching_refs), collapse = " "))

if(length(matching_refs) == 0) {
  stop("Computed reference matrices do not match available references")
} else {
...
temp_archr_dir <- paste0("archr_temp",ids::uuid())
...
doublet_scores <- addDoubletScores(input = ArrowFile,
                                   outDir = temp_archr_dir,
                                   force = TRUE)

proj <- ArchRProject(ArrowFiles = ArrowFile,
                     outputDirectory = temp_archr_dir,
                     copyArrows = TRUE)

archr_names <- getCellNames(proj)
archr_names <- sub(".+#","",archr_names)
new_barcodes <- meta$barcodes[match(archr_names, meta$original_barcodes)]
rownames(proj@cellColData) <- new_barcodes

proj <- filterDoublets(proj)

filtered_barcodes <- getCellNames(proj)
archr_meta <- getCellColData(proj)

n_pre_doublet_filter <- nrow(filtered_meta)

filtered_meta <- filtered_meta %>%
  dplyr::filter(barcodes %in% filtered_barcodes)

n_post_doublet_filter <- nrow(filtered_meta)
n_doublets <- n_post_doublet_filter - n_pre_doublet_filter

filtered_meta <- filtered_meta[match(filtered_meta$barcodes, filtered_barcodes),]
...
#### Add additional metadata to archr project
```{r}
stm("ArchR: Merging metadata")

additional_meta <- setdiff(names(filtered_meta), names(archr_meta))
for(meta_col in additional_meta) {
  proj <- addCellColData(proj,
                         data = filtered_meta[[meta_col]],
                         name = meta_col,
                         cells = filtered_meta$barcodes)
}
```
...
file.rename(ArrowFile, out_arrow)
```

Replace original cell names with barcodes
```{r}
stm(paste0("Updating barcodes in ", out_arrow))
h5_CellNames <- h5ls(out_arrow)
h5_CellNames <- h5_CellNames[h5_CellNames$name == "CellNames",]

walk(1:nrow(h5_CellNames),
     function(i) {
       target <- h5_CellNames$full_name[i]
       target_bcs <- h5read(out_arrow, target)
       new_bcs <- meta$barcodes[match(target_bcs, meta$original_barcodes)]
       h5write(new_bcs, out_arrow, target)
     })
...
#### Clean up ArchR files
```{r}
stm("ArchR: Cleaning up temporary files")

system("rm -r ./tmp/")
system("rm -r ./ArchRLogs/")
system(paste("rm -r", temp_archr_dir))
if(out_dir != "./") {
  file.remove(paste0(in_sample, ".arrow"))
}
rm(proj)
```
...
barcode_conditions <- list("all" = rep(TRUE, nrow(meta)),
                           "singlets" = ifelse(n_post_doublet_filter > 0,
                                               rep(TRUE, n_post_doublet_filter),
                                               FALSE),
                           "doublets" = ifelse(n_doublets > 0, 
                                               rep(TRUE, n_doublets), 
                                               FALSE),
...
barcode_qc_stats <- data.frame(metric = c("Barcodes with > 1k reads",
                                          "Barcodes from Singlets",
                                          "Barcodes from Doublets",
                                          "Barcodes pass QC",
                                          "Barcodes fail QC",
