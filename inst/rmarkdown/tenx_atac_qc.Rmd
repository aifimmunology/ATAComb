...
if(is.null(params$in_pre)) {
  in_pre <- system.file("testdata/outs/fragments.tsv.gz", package = "ATAComb")
...
barcode_qc_stats <- data.frame(metric = c("Barcodes with > 1k reads",
                               ...
                               count = c(nrow(meta),
                                         nrow(filtered_meta),
                                         nrow(meta) - nrow(filtered_meta),
                                         sum(meta$fralt < fralt_cut),
                                         sum(meta$frip < frip_cut),
                                         sum(meta$fritss < fritss_cut),
                                         sum(meta$fralt < fralt_cut & meta$frip < frip_cut & meta$fritss < fritss_cut)))
barcode_qc_stats$percent <- round(barcode_qc_stats$count / barcode_qc_stats$count[1] * 100, 2)
