...
```{r}
meta_out <- file.path(out_dir, paste0(in_batch, "_",in_sample,"_all_metadata.csv.gz"))
stm(paste("Writing unfiltered metadata to", meta_out))
fwrite(meta, meta_out)

filtered_meta_out <- file.path(out_dir, paste0(in_batch, "_",in_sample, "_filtered_metadata.csv.gz"))
...
```{r Write Altius h5}
alt_out_file <- file.path(out_dir, paste0(in_batch, "_",in_sample, "_altius.h5"))
...
out_json <- file.path(out_dir, paste0(in_well, "_atac_qc_metrics.json"))
```
...
```{r}
out_proj <- file.path(out_dir, paste0(in_batch,"_",in_sample, "_saturation_projection.csv.gz"))
