...
if(genome == "hg38") {
  ...
  alt_idx_file <- system.file("reference/hg19_alt_dhs_index.bed.gz", package = "ATAComb")
  peaks_file <- system.file("reference/hg19_GSE123577_filtered.bed.gz", package = "ATAComb")
  gene_bodies_file <- system.file("reference/hg19_ensemble87_gene_bodies_gr.rds", package = "ATAComb")
  tss_file <- system.file("reference/hg19_ensemble87_tss_2kb_gr.rds",package = "ATAComb")
}
...
tss_names <- paste(seqnames(tss_gr),
                   start(tss_gr),
                   end(tss_gr),
                   sep = "_")
tss_list <- list(matrix = list(barcodes = names(barcode_counts),
                               ...
                               shape = c(length(tss_gr),
                                         nrow(filtered_meta)),
                               features = list(id = tss_gr$gene_id,
                                               name = tss_names,
                                               symbol = tss_gr$gene_name)))
rm(tss_dt)
...
gene_bodies_names <- paste(seqnames(gene_bodies_gr),
                           start(gene_bodies_gr),
                           end(gene_bodies_gr),
                           sep = "_")
gene_bodies_list <- list(matrix = list(barcodes = names(barcode_counts),
                                       ...
                                       indptr = c(0, cumsum(barcode_counts)),
                                       shape = c(length(gene_bodies_gr),
                                                 nrow(filtered_meta)),
                                       features = list(id = gene_bodies_gr$gene_id,
                                                       name = gene_bodies_names,
                                                       symbol = gene_bodies_gr$gene_name)))
rm(gene_bodies_dt)
