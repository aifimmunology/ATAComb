...
if(!file.exists(in_sin)) {
  stm(paste0("ERROR: Cannot find IN singlecell.csv or barcode_counts.csv:", in_sin))
  stop()
}
...
} else if(grepl("barcode_counts.csv", in_sin)) {
  stm("Using barcode_counts.csv")
...
for(i in seq_along(cutoffs)) {
  ...
  cut_val <- cutoffs[[i]]
  filtered_meta <- meta[meta[[cut_name]] > cut_val]
}
```
...
window_metas <- map(seq_along(window_offsets),
                   function(x) {
                     ...
                     data.frame(name = gsub(" +","",paste(region_bed$chr, region_bed$start, region_bed$end, sep = "_")),
...
walk(seq_along(window_mat_files),
     function(x) {
       ...
       offsets <- window_offsets[[window_mat_name]]
