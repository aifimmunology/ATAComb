...
if(!dir.exists(in_tenx)) {
  ...
  if(pipeline_type == "arc_v1") {
    in_sum <- file.path(in_tenx, "atac_summary.csv")
  } else {
...
meta$pool_id <- sub("C[0-9]W[0-9]+$","",meta$well_id)
meta$chip_id <- sub("W[0-9]+$","",meta$well_id)
meta$batch_id <- in_batch
meta$pbmc_sample_id <- in_sample
if(grepl("PB([0-9]+)-[0-9]+", in_sample)) {
  meta$sample.sampleKitGuid <- sub("PB([0-9]+)-[0-9]+", "KT\\1", in_sample)
}
```
...
if(pipeline_type %in% c("arc_v1","atac_v1")) {
  ...
  metrics_summary <- read.csv(in_sum)
  metrics_summary$num_fragments <- metrics_summary[["Pre-Normalization total mapped read pairs"]]
  
  # No frac_waste_mitochondrial from cellranger-atac v2.0
  metrics_summary$frac_waste_mitochondrial <- 0
  stm("WARNING: Mitochondrial fragments not computed for cellranger-atac v2.0")
  warning("WARNING: Mitochondrial fragments not computed for cellranger-atac v2.0")
  
  metrics_summary$total_usable_fragments <- metrics_summary[["Pre-Normalization unique fragments in library"]]
}
```
