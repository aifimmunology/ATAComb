...
if(!in_well %in% samplesheet$WellID) {
  stm(paste0("ERROR: Could not find ",in_well, " in SampleSheet WellID column"))
  stop(paste0("ERROR: Could not find ",in_well, " in SampleSheet WellID column"))
}

in_sample <- as.character(samplesheet$SampleID[samplesheet$WellID == in_well])
in_kit <- as.character(samplesheet$sample.sampleKitGuid[samplesheet$WellID == in_well])

stm(paste0("SampleSheet WellID: ", in_well, "; SampleID: ", in_sample))

out_prefix <- file.path(out_dir, paste0(in_pool, "_", in_sample, "_"))
```
...
meta$batch_id <- sub("-.+","",meta$well_id)
meta$pbmc_sample_id <- in_sample
meta$sample.sampleKitGuid <- in_kit
