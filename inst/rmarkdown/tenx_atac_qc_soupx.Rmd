...
reference_datasets <- map(reference_files,
                          readRDS)
reference_datasets <- map(reference_datasets, as.data.frame)
...
walk(seq_along(feature_mat_files),
     function(x) {
       ...
       soup_dt <- dt[!original_barcodes %in% filtered_meta$original_barcodes]
       soup_counts <- table(soup_dt$original_barcodes)
       soup_barcodes <- ids::uuid(length(soup_counts), drop_hyphens = TRUE)
       soup_mat_list <- list(matrix = list(barcodes = soup_barcodes,
                                      data = soup_dt$x,
                                      indices = soup_dt$i - 1,
                                      indptr = c(0, cumsum(soup_counts)),
                                      shape = c(n_features, length(soup_barcodes)),
                                      features = reference_meta[[mat_name]]))
       
       soup_out_file <- paste0(out_prefix, mat_name, "_soup.h5")
       stm(paste("Writing .h5 file to:", soup_out_file))
       write_h5_list(soup_mat_list,
                     soup_out_file)
