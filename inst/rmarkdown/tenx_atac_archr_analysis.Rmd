...
if(n_cells < 200) {
  ...
  padding_zip_file <- system.file("inst/testdata/Padding.arrow.gz", package = "ATAComb")
  file.copy(padding_zip_file, "./Padding.arrow.gz")
  gunzip("Padding.arrow.gz")
  
  proj <- ArchRProject(ArrowFiles = c(arrowFiles, "Padding.arrow"),
                       outputDirectory = temp_dir)
...
```{r}
pad_ArchRProject <- function(proj,
                             n_total = 200) {
  n_cells <- length(proj$cellNames)
  
  n_need <- n_total - n_cells
  padded_cellNames <- c(proj$cellNames, paste0("Padding",1:n_need,sep = "#"))
  padded_Sample <- c(proj$Sample, rep("Padding", n_need))
  
  proj2 <- proj[c(1:n_cells, sample(1:n_cells, n_need, replace = TRUE)),]
  proj2$cellNames <- padded_cellNames
  proj2$Sample <- padded_Sample
  proj2
}

unpad_ArchRProject <- function(proj) {
  proj[proj$Sample != "Padding",]
}
```


```{r}
if(length(proj$cellNames) < 200) {
  proj <- pad_ArchRProject(proj,
                           n_total = 200)
}
```
