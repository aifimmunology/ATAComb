...
if(n_cells < 200) {
  ...
  padding_zip_file <- system.file("testdata/Padding.arrow.gz", package = "ATAComb")
  file.copy(padding_zip_file, "./Padding.arrow.gz")
  system("gunzip Padding.arrow.gz")
...
if(all(atac_cell_counts < 200)) {
  ...
  stm("Some results will be based on unconstrained integration.")
  
  atac_cell_holdout <- atac_cells[small_logi]
  atac_cells <- atac_cells[!small_logi]
  rna_cells <- atac_cells[!small_logi]
...
if(run_co) {
  ...
  proj$co_pred_group <- proj$un_pred_group
  proj$co_pred_score <- proj$un_pred_score
  proj$co_pred_type <- proj$un_pred_type
}
```
